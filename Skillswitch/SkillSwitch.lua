...
function SkillSwitch:OnDocLoaded()
	Apollo.CreateTimer("SkillSwitchLoadTimer", 1, false)
end
...
function SkillSwitch:GetActionButtonPositions()
	...
	if (buttonPositions == nil) then
		buttonPositions = self:GetActionButtonsForgeUI()
	end
	if (buttonPositions == nil) then
...
function SkillSwitch:GetActionButtonsForgeUI()
	local addon = Apollo.GetAddon("ForgeUI_ActionBars")
	if (addon == nil) then
		self:WriteDebug("ForgeUI_ActionBars is not enabled")
		return nil
	end
	local lasButtons = addon.wndActionBar:FindChild("Holder"):GetChildren()
	if (lasButtons == nil or lasButtons[1] == nil or lasButtons[8] == nil) then
		self:WriteDebug("No action buttons found in ForgeUI_ActionBars")
		return nil
	end
	local result = {}
	...
	if (self.settings.enablePathFlyOut) then
		pathButton = addon.wndPathBtn:GetChildren()[1]
